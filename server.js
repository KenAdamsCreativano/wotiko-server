...
app.post("/api/parking/park", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Parked: ${vehicle_number} | Wing: ${parking_area} | Phone: ${guest_phone}`
    );
    res.status(201).json({
      success: true,
      docId: ref.id,
...
app.post("/car-ready", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ MSG 4 (OTP plain text) → ${phone} | OTP: ${otp} | Car: ${carNumber}`
    );
...
app.post("/verify-otp", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ MSG 5 (handover_complete) → ${phone} | Car: ${result.carNumber}`
    );
    ...
    if (docId) {
      ...
      if (doc.exists && doc.data().Retrieve_request_time) {
        const diffMins = Math.round(
          (Date.now() - doc.data().Retrieve_request_time.toDate().getTime()) /
            60000
        );
        update.Retrieve_time = `${diffMins} min`;
      }
